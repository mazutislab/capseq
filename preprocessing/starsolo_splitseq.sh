...
genomic=( $(find $PWD/raw/fastq/ -type f -name "*_1.fastq.gz") )

# get barcode dirs to array
barcode=( $(find $PWD/raw/fastq/ -type f -name "*_2.fastq.gz") )

# run starsolo
for i in "${!genomic[@]}"; do
    
    # get parent directory
    prefix=$(basename -s "_1.fastq.gz" "${genomic[$i]}")
    ...
    STAR \
        ...
        --soloCBwhitelist $PWD/raw/barcodes/CB23.txt $PWD/raw/barcodes/CB23.txt $PWD/raw/barcodes/CB1.txt \
        --runThreadN 16 \
